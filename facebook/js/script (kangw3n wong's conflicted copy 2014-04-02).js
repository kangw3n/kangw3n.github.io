...
function requestClick1(){//Rewrite innerHTML
    ...
    request.open('GET', 'https://dl.dropboxusercontent.com/s/78ayglthms7p9ix/data.json');
...
function requestClick(){
    $.getJSON('https://dl.dropboxusercontent.com/s/78ayglthms7p9ix/data.json' , function(data){
        console.log(data);
        var output = '<ul>';
    ...
    if (searchField !== "" || 0) {
        ...
        for (var i=0; i<friendName.length; i++) { 
            ...
            if (friendname.search(myExp) != -1) {// -1 if no match is found
                ...
                friendsnumber = $("#friendscontent > div").length;
                $('#frenno').html(friendsnumber);
                 message = "Your have found " + friendsnumber + " friends";
                alertify.log( message, "custom" );
            } else {
...
function requestClick2(){
    ...
    request.open('GET', 'https://dl.dropboxusercontent.com/s/212hp2t7evosxgk/data.txt');
    console.log(request);
    request.onreadystatechange = function() {
        if ((request.readyState===4) && (request.status===200)) {
